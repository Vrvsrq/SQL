...
/* 
Создаем процедуру, которая будет выводить данные о мероприятиях, либо не исполненных, 
либо исполненных до указанной даты в параметре процедуры, связанных с указанным 
документом в параметре процедуры и всеми его родительскими документами.
*/
create or alter procedure dbo.RPT_Subscrs_Docs
	@F_Docs int, -- Идентификатор документа
    @D_Date date   -- Дата, на которую ищем мероприятия
as
begin
	set nocount on;
	select
		s.C_Number as [Номер ЛС],
		s.C_FirstName + ' ' + C_SecondName as [ФИО],
		format(s.D_BirthDate, 'dd.MM.yyyy') as [Дата Рождения],
		d.C_Number as [Номер Документа],
		format(d.D_Date, 'dd.MM.yyyy') as [Дата Документа],
		case when datediff(yyyy, s.D_BirthDate,d.D_Date) > 18 then 'Да' else 'Нет' end  [Совершеннолетний],
		a.C_Name as [Мероприятие],
		format(a.D_NeedToDo_Date, 'dd.MM.yyyy') as [Плановая Дата Исполнения],
		format(a.D_Done_Date, 'dd.MM.yyyy') as [Дата Исполнения],
		case when a.B_Done = 1 then 'Да' else 'Нет' end [Признак Исполнено]
	from
		dbo.SD_Subscrs as s
	inner join
		dbo.DD_Docs as d on d.F_Subscr = s.LINK
	inner join 
		dbo.DD_Docs_Assignments as a on a.F_Docs = d.LINK
	where
		(a.D_Done_Date <= @D_Date or a.D_Done_Date is null)
	and (a.F_Docs = @F_Docs or a.F_Docs = (select F_Docs from 
		dbo.DD_Docs where LINK = @F_Docs))
end;